...
def generate_params_comparison_table(  # noqa: C901
    scalping_params: dict[str, Any] | None,
    swing_params: dict[str, Any] | None,
    ...
    # 安全获取参数值
    def safe_get(params: dict[str, Any] | None, key: str, default: str = "N/A") -> str:
    ...
    scalping_trailing = (
        "✅"
        if scalping_params and scalping_params.get("trailing_stop_enabled")
        else "❌"
    )
    swing_trailing = (
        "✅" if swing_params and swing_params.get("trailing_stop_enabled") else "❌"
    )
    ...
    if isinstance(scalping_density_val, (int, float)):
        if scalping_density_val < 2:
            scalping_density = (
                f"⚠️ {scalping_density_val:.2f} (异常低，请检查Phase 1统计)"
            )
        else:
            scalping_density = f"{scalping_density_val:.2f}"
    else:
        scalping_density = (
            str(scalping_density_val) if scalping_density_val is not None else "N/A"
        )
    ...
    if isinstance(swing_density_val, (int, float)):
        ...
        swing_density = (
            str(swing_density_val) if swing_density_val is not None else "N/A"
        )
...
def generate_profit_comparison_table(phase_data: dict[str, Any]) -> str:
    ...
    scalping_improvement_pct = (
        (scalping_improvement / p2_scalping_total * 100) if p2_scalping_total > 0 else 0
    )
    swing_improvement = p4_swing_total - p2_swing_total
    swing_improvement_pct = (
        (swing_improvement / p2_swing_total * 100) if p2_swing_total > 0 else 0
    )

    scalping_icon = (
        "📈" if scalping_improvement > 0 else "📉" if scalping_improvement < 0 else "➡️"
    )
    scalping_color = (
        "#28a745"
        if scalping_improvement > 0
        else "#dc3545"
        if scalping_improvement < 0
        else "#6c757d"
    )
    swing_icon = (
        "📈" if swing_improvement > 0 else "📉" if swing_improvement < 0 else "➡️"
    )
    swing_color = (
        "#28a745"
        if swing_improvement > 0
        else "#dc3545"
        if swing_improvement < 0
        else "#6c757d"
    )
...
def generate_signal_weights_comparison_table(
    scalping_weights: dict[str, Any] | None,
    swing_weights: dict[str, Any] | None,
    old_scalping_weights: dict[str, Any] | None = None,
    old_swing_weights: dict[str, Any] | None = None,
) -> str:
    """生成信号分权重对比表HTML.
    ...
    def extract_weights(weight_dict: dict[str, Any] | None) -> dict[str, Any]:
        """提取权重，兼容新旧两种格式."""
        if not weight_dict:
            return {}
        # 如果有'weights'键，使用它（旧格式）
        if "weights" in weight_dict:
            return weight_dict["weights"]  # type: ignore[return-value]
    ...
    for key, label, value in scalping_items:
        ...
        if change != 0 and old_value > 0:
            ...
            change_html = (
                f'<br><small style="color: {change_color};">({change:+.0f})</small>'
            )
