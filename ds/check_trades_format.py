...
def check_trades_format(model_name):
    ...
    trades_file = (
        Path(__file__).parent / "trading_data" / model_name / "trades_history.csv"
    )
        ...
        with open(trades_file, encoding="utf-8") as f:
            ...
            for idx, trade in enumerate(trades[-5:], start=max(1, total - 4)):
                ...
                if actual_fields != len(fieldnames):
                    print(
                        f"    ⚠️  字段数不匹配: 期望{len(fieldnames)}, 实际{actual_fields}"
                    )
                ...
                for field in key_fields:
                    ...
                    if value is None:
                        # 尝试找相似字段
                        similar = [
                            f for f in fieldnames if field in f or f.strip() == field
                        ]
                        if similar:
                            value = trade.get(similar[0])
                            if value is None:
                                missing_fields.append(
                                    f"{field} (找到相似: {similar[0]})"
                                )
                        else:
            ...
            if duplicates:
                ...
                for orig_idx, dup_idx in duplicates[:5]:
                    ...
                    dup = trades[dup_idx]
                    print(
                        f"    #{orig_idx + 1} 和 #{dup_idx + 1}: {orig.get('币种')} {orig.get('方向')} "
                        f"@{orig.get('平仓时间', '未平仓')}"
                    )
                if len(duplicates) > 5:
            ...
            if partial_closes:
                ...
                for orig_idx, dup_idx in partial_closes:
                    orig = trades[orig_idx]
                    base_key = (
                        f"{orig.get('币种')}_{orig.get('方向')}_{orig.get('开仓时间')}"
                    )
