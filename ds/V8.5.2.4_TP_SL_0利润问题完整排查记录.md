...
## 🔴 V8.5.2.4.65 - 波动幅度判断法（2025-11-19）

### V8.5.2.4.64回测结果

**修复成功，但发现新问题**：
```
Phase 2 TP/SL测试:      12.24% ✅（超短线）
Phase 2 TP/SL测试:      13.78% ✅（波段）
Phase 2 参数组合测试:    0.53% ❌（差距-96%）
...
统计：stop_loss_prob_90%占92%
```

### 根本原因：V8.5.2.4.17概率算法缺陷
...
**问题**：
- Phase 2 TP/SL参数极端（TP=30倍, SL=1.5倍 → 比例20:1）
- 距离权重过大，忽略实际波动方向
- 即使下跌幅度18.5% > 上涨幅度6.8%，仍判定90%是SL ❌
...
if upward_amplitude > downward_amplitude:
    exit_price = take_profit  # TP先触发
else:
    exit_price = stop_loss    # SL先触发
...
**预期效果**：
```
Phase 2 参数组合测试: 0.53% → 10-12% (+1825%)
...
V8.5.2.4.64: 修复同时触发时exit_method未定义的Bug
  ↓ 回测发现
V8.5.2.4.65: 修复V8.5.2.4.17概率算法（用波动幅度法替代）
  ↓ 最终解决
✅ 预期Phase 2利润10-12%+
```
...
- `V8.5.2.4.62_None值Fallback修复.md` - None值修复
- `V8.5.2.4.63_增强调试日志.md` - 增强调试（发现同时触发bug）
- `V8.5.2.4.64_同时触发TP_SL的0利润Bug修复.md` - 0.00%修复
- `V8.5.2.4.65_波动幅度判断法修复.md` - **概率算法替换**
...
4. **V8.5.2.4.17概率算法缺陷**（距离比例 vs 波动幅度）

通过5次迭代调试（V8.5.2.4.61-65），逐步揭示深层问题，最终完全修复。
