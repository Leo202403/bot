            ...
            'ai_decisions': [
                {
                    ...
                    'action': 'open_long',
                    'reason': 'AI的原始决策理由...',
                    'result': 'timing_issue',  # 或 correct, false_signal
                    'pnl': 0.5
                },
                ...
            ],
...
- ✅ Correct: {analysis_data['entry_summary']['correct']} ({analysis_data['entry_summary']['correct']/max(analysis_data['entry_summary']['total'],1)*100:.0f}%)
- ⚠️ Timing Issue: {analysis_data['entry_summary']['timing_issue']} ({analysis_data['entry_summary']['timing_issue']/max(analysis_data['entry_summary']['total'],1)*100:.0f}%)
- ❌ False Signal: {analysis_data['entry_summary']['false_signal']} ({analysis_data['entry_summary']['false_signal']/max(analysis_data['entry_summary']['total'],1)*100:.0f}%)

## Exit Performance
- Total: {analysis_data['exit_summary']['total']} exits
- ⏰ Premature: {analysis_data['exit_summary']['premature']} ({analysis_data['exit_summary']['premature']/max(analysis_data['exit_summary']['total'],1)*100:.0f}%)
- ⏱️ Delayed: {analysis_data['exit_summary']['delayed']} ({analysis_data['exit_summary']['delayed']/max(analysis_data['exit_summary']['total'],1)*100:.0f}%)
- ✅ Optimal: {analysis_data['exit_summary']['optimal']} ({analysis_data['exit_summary']['optimal']/max(analysis_data['exit_summary']['total'],1)*100:.0f}%)
...
<div style="background: #fff; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 5px solid #2196f3;">
    <h3 style="color: #2196f3; margin: 0 0 15px 0;">🤖 AI自我反思</h3>
    
    <div style="margin: 15px 0;">
        <h4 style="color: #4caf50; margin: 10px 0;">✅ 开仓教训</h4>
    ...
    <div style="margin: 15px 0;">
        <h4 style="color: #ff9800; margin: 10px 0;">🚪 平仓教训</h4>
        <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <div style="margin: 15px 0;">
        <h4 style="color: #f44336; margin: 10px 0;">📉 错过机会教训</h4>
        <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <div style="margin: 15px 0; padding: 15px; background: #e3f2fd; border-radius: 5px;">
        <h4 style="color: #1976d2; margin: 0 0 10px 0;">🎯 明日改进计划</h4>
    ...
    <p style="margin: 10px 0; color: #666; font-size: 0.9em;">
        生成时间: {ai_insights['generated_at']} | 
        Token使用: {ai_insights['tokens_used']} | 
        成本: ${ai_insights['cost_usd']:.6f}
    </p>
</div>
"""
```
...
if entry_analysis or exit_analysis:
    try:
        ...
        print(f"  ✓ Entry Lessons: {len(ai_insights['entry_lessons'])}")
        print(f"  ✓ Exit Lessons: {len(ai_insights['exit_lessons'])}")
        print(f"  ✓ Missed Lessons: {len(ai_insights['missed_lessons'])}")
        print(f"  ✓ Improvements: {len(ai_insights['improvements'])}")
        print(f"  ✓ Cost: ${ai_insights['cost_usd']:.6f}")
